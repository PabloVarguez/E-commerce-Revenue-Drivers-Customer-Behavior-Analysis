# E-commerce-Revenue-Drivers-Customer-Behavior-Analysis

## Key Insight

Revenue growth is primarily driven by an increase in the number of orders, while average order value remains stable.  
Additionally, over 90% of customers are one-time buyers, indicating a strong opportunity for retention strategies.

## English

### Project Overview
This project analyzes sales and customer behavior using the Brazilian E-Commerce Public Dataset by Olist.  
The objective is to identify the main drivers of revenue and provide actionable business insights.

---

### Business Objective
- Understand what drives revenue growth
- Analyze customer purchasing behavior
- Identify key product categories
- Provide data-driven business recommendations

---

### Tools & Technologies
- Python
- Pandas
- NumPy
- Matplotlib / Seaborn

---

### Dataset
- Source: Brazilian E-Commerce Public Dataset (Olist)
- Data includes:
  - Orders
  - Customers
  - Products
  - Order items

---

### Key Metrics
- Total Revenue
- Monthly Revenue
- Number of Orders
- Average Order Value (AOV / Ticket promedio)
- New vs Returning Customers
- Revenue by Category

---

### Key Insights

- Revenue growth is driven primarily by an increase in the number of orders.
- Average order value remains relatively stable over time.
- Over 90% of customers make only one purchase → low retention rate.
- Top revenue categories:
  - beleza_saude
  - relogios_presentes
  - cama_mesa_banho
- These categories perform strongly due to high sales volume rather than higher prices.

---

### Business Recommendations

- Implement customer retention strategies (loyalty programs, incentives)
- Increase marketing efforts to acquire new customers
- Apply cross-selling and up-selling strategies
- Evaluate pricing strategies in high-performing categories

---

## Español

### Descripción del Proyecto
Este proyecto analiza el comportamiento de ventas y clientes utilizando el Brazilian E-Commerce Public Dataset by Olist.  
El objetivo es identificar los principales factores que impulsan el revenue y generar insights accionables para la toma de decisiones.

---

### Objetivo de Negocio
- Entender qué impulsa el crecimiento del revenue
- Analizar el comportamiento de compra de los clientes
- Identificar las categorías clave
- Proponer acciones estratégicas basadas en datos

---

### Herramientas
- Python
- Pandas
- NumPy
- Matplotlib / Seaborn

---

### Dataset
- Fuente: Olist Brazilian E-Commerce Dataset
- Tablas utilizadas:
  - Orders
  - Customers
  - Products
  - Order Items

---

### Métricas Clave
- Revenue total y mensual
- Número de órdenes
- Ticket promedio
- Clientes nuevos vs recurrentes
- Revenue por categoría

---

### Principales Insights

- El crecimiento del revenue está impulsado por el aumento en el número de órdenes.
- El ticket promedio se mantiene estable.
- Más del 90% de los clientes realizan una sola compra → baja retención.
- Categorías principales:
  - beleza_saude
  - relogios_presentes
  - cama_mesa_banho
- Estas categorías destacan por volumen de ventas, no por precios elevados.

---

### Recomendaciones

- Implementar estrategias de fidelización
- Aumentar adquisición de clientes
- Aplicar cross-selling y up-selling
- Evaluar ajustes de precios en categorías clave

